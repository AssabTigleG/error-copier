 ...
 */
export function escapeCsvField(field: string | number | undefined): string {
 ...
 */
export function getDefaultExcludes(): vscode.GlobPattern | undefined {
 ...
 */
export async function estimateTotalFiles(uris: vscode.Uri[]): Promise<number> {
    let count = 0;
    const limit = 500;
    for (const u of uris) {
        try {
 ...
 */
export function processDiagnosticsForReportGrouping(
    diagnostics: RawDiagnosticInfo[],
    docLinesCache: Map<string, string[]>,
    options?: { linesBefore?: number; linesAfter?: number; threshold?: number }
): FormattedReportGroup[] {
    if (!diagnostics.length) return [];

    const config = vscode.workspace.getConfiguration('errorcontextcopier');
    const linesBefore = options?.linesBefore ?? config.get<number>('contextLinesBefore', 1);
    const linesAfter = options?.linesAfter ?? config.get<number>('contextLinesAfter', 1);
    const threshold = options?.threshold ?? config.get<number>('groupingLineThreshold', 2);

    const sortedDiags = [...diagnostics].sort((a, b) => {
        const uriA = a.fileUri.toString();
        const uriB = b.fileUri.toString();
        if (uriA < uriB) return -1;
        if (uriA > uriB) return 1;
 ...
 */
export async function collectAndProcessDiagnostics(
    targetUris: vscode.Uri[],
    scanTitle: string,
    severitiesOverride?: vscode.DiagnosticSeverity[]
): Promise<FormattedReportGroup[] | null> {
    const config = vscode.workspace.getConfiguration('errorcontextcopier');
    const configuredSeverityStrings = config.get<string[]>('includeSeverities', ['Error', 'Warning']);
    const severities = severitiesOverride ?? configuredSeverityStrings.map(s => SEVERITY_MAP[s]).filter(s => s !== undefined);
    const ignoreCodes = config.get<(string | number)[]>('ignoredErrorCodes', []);
    const ignoreMsgs = config.get<string[]>('ignoredErrorMessages', []);
    const chunkSize = config.get<number>('asyncChunkSize', 50);
 ...
 */
export function generateMarkdownReport(formattedReportGroups: FormattedReportGroup[]): string {
 ...
 */
export function generateJsonReport(formattedReportGroups: FormattedReportGroup[]): string {
 ...
 */
export function generateHtmlFileReport(formattedReportGroups: FormattedReportGroup[]): string {
 ...
 */
export function generateCsvReport(formattedReportGroups: FormattedReportGroup[]): string {
 ...
 */
export function generateAiPrompt(formattedReportGroups: FormattedReportGroup[]): string {
