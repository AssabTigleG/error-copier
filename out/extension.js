...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('errorcontextcopier.tree.fixAllInFile', async (item) => {
        if (item?.fileUri) {
            try {
                ...
                const sourceActions = await vscode.commands.executeCommand('vscode.executeCodeActionProvider', item.fileUri, wholeDocRange, vscode.CodeActionKind.SourceFixAll.value);
                ...
                for (const diag of diags) {
                    const actions = await vscode.commands.executeCommand('vscode.executeCodeActionProvider', item.fileUri, diag.range, vscode.CodeActionKind.QuickFix.value);
                ...
                vscode.window.showInformationMessage(appliedCount > 0 ? `Applied ${appliedCount} auto-fix(es) in ${item.label}.` : `No automatic fixes available for ${item.label}.`);
            }
            catch (e) {
                vscode.window.showErrorMessage(`Failed to auto-fix ${item.label}: ${e}`);
            }
        }
    }), vscode.commands.registerCommand('errorcontextcopier.tree.copyFileErrors', async (item) => {
        if (item?.fileUri) {
