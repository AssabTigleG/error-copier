...
const SEVERITY_TO_STRING_MAP = {
    [vscode.DiagnosticSeverity.Error]: "Error",
    [vscode.DiagnosticSeverity.Warning]: "Warning",
    [vscode.DiagnosticSeverity.Information]: "Information",
    [vscode.DiagnosticSeverity.Hint]: "Hint"
};
let reportPanel = undefined;
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('errorcontextcopier.refreshDiagnosticsView', () => diagnosticTreeDataProvider.refresh()));
    ...
    context.subscriptions.push(vscode.commands.registerCommand('errorcontextcopier.view.scanWorkspaceAndShowPanel', async () => {
        ...
        if (reportData && reportData.length > 0)
            createOrShowReportPanel(context.extensionUri, reportData);
        else if (reportData)
...
async function collectAndProcessDiagnostics(targetUris, scanTitle) {
    ...
    const severities = config.get('includeSeverities', ['Error']).map(s => SEVERITY_MAP[s]).filter(s => s !== undefined);
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: scanTitle, cancellable: true }, async (prog, token) => {
        ...
        for (const u of targetUris) {
            if (token.isCancellationRequested)
                break;
            try {
                (await vscode.workspace.fs.stat(u)).type === vscode.FileType.Directory ? folders.push(u) : files.push(u);
            }
            catch (e) {
        ...
        const processFile = async (fUri) => {
            if (token.isCancellationRequested)
                return;
            processed++;
            prog.report({ message: `Processing: ${vscode.workspace.asRelativePath(fUri)}`, increment: (1 / Math.max(1, estimate)) * 100 });
            try {
                let actionable = vscode.languages.getDiagnostics(fUri).filter(d => severities.includes(d.severity));
                if (ignoreCodes.length > 0)
                    actionable = actionable.filter(e => !e.code || !ignoreCodes.some(ic => String(ic) === (typeof e.code === 'object' ? String(e.code.value) : String(e.code))));
                if (ignoreMsgs.length > 0)
                    actionable = actionable.filter(e => !ignoreMsgs.some(p => { try {
                        return p.startsWith('/') && p.lastIndexOf('/') > 0 ? new RegExp(p.substring(1, p.lastIndexOf('/')), p.substring(p.lastIndexOf('/') + 1)).test(e.message) : e.message.includes(p);
                    }
                    catch {
                        return false;
                    } }));
                if (actionable.length > 0) {
                    if (!linesCache.has(fUri.toString()))
                        linesCache.set(fUri.toString(), (await vscode.workspace.openTextDocument(fUri)).getText().split(/\r?\n/));
                    for (const e of actionable)
                        diagsForProc.push({ filePath: vscode.workspace.asRelativePath(fUri, false), fileUri: fUri, message: e.message, startLineZeroIndexed: e.range.start.line, endLineZeroIndexed: e.range.end.line, code: typeof e.code === 'object' ? e.code.value : e.code, severity: e.severity, range: e.range });
                }
            }
            catch (e) {
        ...
        for (const d of folders) {
            ...
            for (const fInD of await vscode.workspace.findFiles(new vscode.RelativePattern(d, '**/*'), getDefaultExcludes()))
                await processFile(fInD);
        }
    });
    if (cancelled)
        return null;
 ...
 */
function processDiagnosticsForReportGrouping(diagnostics, docLinesCache) {
    if (!diagnostics.length)
        return [];
    const config = vscode.workspace.getConfiguration('errorcontextcopier');
    const linesBefore = config.get('contextLinesBefore', 1);
    const linesAfter = config.get('contextLinesAfter', 1);
    const threshold = config.get('groupingLineThreshold', 2);
 ...
 */
function createOrShowReportPanel(extensionUri, reportData) {
    ...
    reportPanel = vscode.window.createWebviewPanel('errorContextReport', 'Error Context Report', col || vscode.ViewColumn.One, { enableScripts: true, localResourceRoots: [vscode.Uri.joinPath(extensionUri, 'webview-ui')] });
    ...
    reportPanel.webview.onDidReceiveMessage(async (msg) => {
        if (msg.command === 'navigateTo') {
            ...
            try {
                ...
                vscode.window.showErrorMessage(`Open fail: ${filePath}. ${e}`);
            }
        }
        else if (msg.command === 'webviewReady')
            reportPanel?.webview.postMessage({ command: 'loadData', data: reportData });
        else if (msg.command === 'copyMarkdownToClipboard')
            await copyReportToClipboard(generateMarkdownReport(msg.data), "Markdown", msg.data.length);
    });
