...
class ReportPanelManager {
    ...
    async autoFixLocation(filePath, line) {
        try {
            ...
            const actions = await vscode.commands.executeCommand('vscode.executeCodeActionProvider', uri, range, vscode.CodeActionKind.QuickFix.value);
    ...
    async fixAllInFile(filePath) {
        try {
            ...
            const sourceActions = await vscode.commands.executeCommand('vscode.executeCodeActionProvider', uri, wholeDocRange, vscode.CodeActionKind.SourceFixAll.value);
            ...
            for (const diag of diags) {
                const actions = await vscode.commands.executeCommand('vscode.executeCodeActionProvider', uri, diag.range, vscode.CodeActionKind.QuickFix.value);
                if (actions && actions.length > 0) {
            ...
            vscode.window.showInformationMessage(appliedCount > 0 ? `Applied ${appliedCount} auto-fix(es) in ${path.basename(filePath)}.` : `No automatic fixes available for ${path.basename(filePath)}.`);
        }
        catch (e) {
            vscode.window.showErrorMessage(`Failed to auto-fix file: ${e}`);
        }
    }
    async fixAllInScope(filePaths) {
        ...
        await vscode.window.withProgress({
            ...
            for (const fp of filePaths) {
                ...
                try {
                    ...
                    if (sourceActions) {
                        for (const act of sourceActions) {
                            if (act.edit && await vscode.workspace.applyEdit(act.edit))
                                totalFixed++;
                        }
                    }
                    const diags = vscode.languages.getDiagnostics(uri);
                    for (const diag of diags) {
                        ...
                        if (actions && actions.length > 0) {
                            ...
                            if (preferred && preferred.edit) {
                                if (await vscode.workspace.applyEdit(preferred.edit))
                                    totalFixed++;
                            }
                        }
                    }
